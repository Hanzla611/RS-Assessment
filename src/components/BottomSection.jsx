...
function BottomSection({ toggleDrawer, userData }) {
  return (
    <div>
      <table className="table text-sm w-full will-change-scroll">
          ...
          {userData.users?.length > 0 ? (
            userData?.users?.slice(0, 5).map((user) => {
              return (
                <tr className="row">
                  <td
                    className="p-3 font-medium capitalize flex items-center"
                    style={{
                      borderLeft:
                        user.id === 1
                          ? "5px solid #50C878"
                  ...
                  <td className="px-6 py-4">
                    ...
                    <span className="bg-violet-900 text-white p-1 rounded-full w-2 h-2 ml-1">
                      +4
                    </span>
                  </td>
                  <td
